...
class CloneChannelsMixin:
    ...
    async def connect_forced_join_command(self, update:Update, context:ContextTypes.DEFAULT_TYPE):
        """Connect a Forced Join group/channel without sharing /connectgroup storage."""
        owner=self.owner(context)
        user=update.effective_user
        chat=update.effective_chat
        message=update.effective_message
        ...
        if not chat or chat.type not in {"group", "supergroup", "channel"}:
            await message.reply_text(
                "❌ Send /connectforcedjoin inside the target group/channel.\n\n"
                "The bot must be an administrator with permission to invite users."
            )
            return

        try:
            # Never reuse /connectgroup storage. A chat already connected there
            # is rejected instead of being silently duplicated.
            connected=await get_channels(owner)
            if any(int(x.get("chat_id", 0) or 0)==int(chat.id) for x in (connected or [])):
                await message.reply_text(
                    "❌ This group/channel is already connected with /connectgroup.\n\n"
            ...
            # Forced Join has its own collection and schema. access_chat_id=0
            # keeps this connector independent from /connectgroup and the
            # per-access-chat editor until an access chat is explicitly set.
            await upsert_required(
                owner,
                0,
                chat.id,
                chat.title or "Group/Channel",
                chat.type,
                invite.invite_link,
            )
            ...
            await message.reply_text(
                ...
                f"Name: {chat.title or 'Group/Channel'}\n"
                f"Type: {chat.type}\n"
                f"ID: {chat.id}\n"
                "Invite link: Working ✅\n\n"
                "It is now available in Group Manager → Forced Join.\n"
                "This connection is completely separate from /connectgroup."
            )
        except Exception as exc:
            logger.exception("Forced Join connection failed owner=%s chat=%s", owner, getattr(chat, "id", None))
            await message.reply_text(
