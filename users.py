...
from handlers.clone.payment.delivery import ClonePaymentDeliveryMixin
...
async def handle(self, update, context, q, owner, staff, a, role):
    if a == 'a_users':
        if was_expired:
            try:
                delivery = getattr(self, 'deliver_subscription_access', None)
                if delivery:
                    await delivery(owner, user_id, {
                ...
                try:
                    logger.warning(
                        'Admin extension access restore failed owner=%s user=%s group=%s: %s',
                        owner, user_id, gid, exc,
                    )
                except Exception:
                    pass
    ...
    if a.startswith('a_user_group_extend_'):
        ...
        # Point 3: remember whether the subscription was already expired before extending.
        # Only expired subscriptions need access/invite restoration; active extensions keep
        # the existing delivery behavior unchanged.
        was_expired = True
        try:
            expiry_before = sub.get('expiry_date')
            if expiry_before:
                was_expired = expiry_before <= datetime.now(timezone.utc)
        except (TypeError, ValueError):
            was_expired = True
